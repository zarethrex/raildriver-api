...
def find_tsclassic() -> pathlib.Path:
    ...
    _railworks_path = typing.cast("str", _railworks_path)
    return pathlib.Path(_railworks_path)
...
def get_raildriver_dll(*, use_x86: bool = False) -> pathlib.Path:
    ...
    if not _railworks_plugins_directory.exists():
        raise FileNotFoundError(
            f"Location '{_railworks_plugins_directory}' does not exist."
        )

    if not (
        _dll_location := _railworks_plugins_directory.joinpath(_dll_file_name)
    ).exists():
        raise FileNotFoundError(
